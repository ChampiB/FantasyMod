...
public class TextFilterClient implements AutoCloseable {
   ...
   CompletableFuture<FilteredText<String>> requestMessageProcessing(GameProfile p_10137_, String p_10138_, TextFilterClient.IgnoreStrategy p_10139_, Executor p_10140_) {
      return p_10138_.isEmpty() ? CompletableFuture.completedFuture(FilteredText.f_215167_) : CompletableFuture.supplyAsync(() -> {
         ...
         try {
            ...
            if (flag) {
               return FilteredText.m_215181_(p_10138_);
            } else {
               String s = GsonHelper.getAsString(jsonobject1, "hashed", (String)null);
               if (s == null) {
                  return FilteredText.m_215186_(p_10138_);
               } else {
                  int i = GsonHelper.getAsJsonArray(jsonobject1, "hashes").size();
                  return p_10139_.shouldIgnore(s, i) ? FilteredText.m_215186_(p_10138_) : new FilteredText<>(p_10138_, s);
               }
            }
         } catch (Exception exception) {
   ...
   class PlayerContext implements TextFilter {
      ...
      public CompletableFuture<List<FilteredText<String>>> processMessageBundle(List<String> p_10190_) {
         List<CompletableFuture<FilteredText<String>>> list = p_10190_.stream().map((p_10195_) -> {
      ...
      public CompletableFuture<FilteredText<String>> processStreamMessage(String p_10186_) {
